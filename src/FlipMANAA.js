...
const MANA_FLIP_ADDRESS = "0x4bf9D2EBC4c57B9B783C12D30076507660B58b3a";
const OSM_ADDRESS = "0x8067259EA630601f319FccE477977E55C6078C13";
...
const flipContract = new ethers.Contract(MANA_FLIP_ADDRESS, flipABI, provider);
...
function FlipMANAA() {
    ...
    async function getMANAPrice(block) {
    ...
    function subscribe() {
        ...
        provider.on(filterAll, async (log) => {
            ...
            let price = await getMANAPrice(log.blockNumber);
    ...
    async function calcExpectedLoss() {
        ...
        let price = await getMANAPrice(latestBlock);
        let lossExp = 0;
    ...
    const auctionList = auctions.map(function(auction){
        if (active === 0 || active === auction["id"]) {
            if (auction["type"] === "KICK") {
                return <div className="event" onClick={() => updateActive(auction["id"])}>KICK @ block {auction["block"]} | ID: {auction["id"]} | lot: {auction["lot"]} mana @ ${auction["price"]}(${(auction["lot"]*auction["price"]).toFixed(2)}) | tab: {auction["tab"]} dai | <a href={"https://etherscan.io/tx/" + auction["hash"]} target="_blank" rel="noopener noreferrer">link</a></div>
            } else if (auction["type"] === "TEND") {
                return <div className="event" onClick={() => updateActive(auction["id"])}>TEND @ block {auction["block"]} | ID: {auction["id"]} | lot: {auction["lot"]} mana @ ${auction["price"]}(${(auction["lot"]*auction["price"]).toFixed(2)}) | bid: {auction["bid"]} dai | rate: {auction["diff"]}% | <a href={"https://etherscan.io/tx/" + auction["hash"]} target="_blank" rel="noopener noreferrer">link</a></div>
            } else if (auction["type"] === "DEAL") {
                return <div className="event" onClick={() => updateActive(auction["id"])}>DEAL @ block {auction["block"]} | ID: {auction["id"]} | lot: {auction["lot"]} mana @ ${auction["price"]}(${(auction["lot"]*auction["price"]).toFixed(2)}) | winning bid: {auction["bid"]} dai | rate: {auction["diff"]}% | <a href={"https://etherscan.io/tx/" + auction["hash"]} target="_blank" rel="noopener noreferrer">link</a></div>
            } else if (auction["type"] === "DENT") {
                return <div className="event" onClick={() => updateActive(auction["id"])}>DENT @ block {auction["block"]} | ID: {auction["id"]} | lot: {auction["lot"]} mana @ ${auction["price"]}(${(auction["lot"]*auction["price"]).toFixed(2)}) | bid: {auction["bid"]} dai | rate: {auction["diff"]}% | <a href={"https://etherscan.io/tx/" + auction["hash"]} target="_blank" rel="noopener noreferrer">link</a></div>
            } 
        }
    })
    ...
    const risky = auctionRecords.map(function(auction){
        if (auction && auction["last"] === "TEND") {
            if (auction["diff"] && auction["diff"] < -33){
                return <p>ID: {auction["id"]} | lot: {auction["lot"]} mana @ ${auction["price"]}(${(auction["lot"]*auction["price"]).toFixed(2)}) | last bid: {auction["bid"]} dai | rate: {auction["diff"]}% </p>
            }
        }
    })
